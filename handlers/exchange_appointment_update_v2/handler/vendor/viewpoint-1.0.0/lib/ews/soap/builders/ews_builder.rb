  ...
  class EwsBuilder
    ...
    def build!(opts = {}, &block)
      @nbuild.Envelope(NAMESPACES) do |node|
        ns = node.parent.namespace_definitions.find {|ns| ns.prefix == NS_SOAP} ## Added because of a bug with the java version of nokogiri
        node.parent.namespace = parent_namespace(node)
        node[ns.prefix].Header { # ns.prefix added because java version of nokogiri wasn't added it without explicitly defining it
        ...
        node[ns.prefix].Body { # ns.prefix added because java version of nokogiri wasn't added it without explicitly defining it
    ...
    # 2016-04-28: Added info, found in online version of this file
    def recurrence!(item)
    ...
    def days_of_week!(item)
      nbuild[NS_EWS_TYPES].DaysOfWeek(item[:text])
    end

    def day_of_week_index!(item)
      nbuild[NS_EWS_TYPES].DayOfWeekIndex(item[:text])
    end

    def month!(item)
      nbuild[NS_EWS_TYPES].Month(item[:text])
    end

    def day_of_month!(item)
      nbuild[NS_EWS_TYPES].DayOfMonth(item[:text])
    end

    def first_day_of_week!(item)
      nbuild[NS_EWS_TYPES].FirstDayOfWeek(item[:text])
    end
    ...
    def start_date_r!(item)
      nbuild[NS_EWS_TYPES].StartDate(item[:text])
    end

    def end_date_r!(item)
      nbuild[NS_EWS_TYPES].EndDate(item[:text])
    end
    # 2016-04-28: End of Adds
