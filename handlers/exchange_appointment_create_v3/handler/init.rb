...
class ExchangeAppointmentCreateV3
  ...
  def execute()
    ...
    puts @parameters['required_attendees'].inspect
    ...
    created_item = calendar.create_item(item)

    if created_item == nil
      raise StandardError, "There was an unexpected error encountered when attempting to create the appointment with the inputted details"
    end

    <<-RESULTS
<results>
  <result name="id">#{created_item.id}</result>
</results>
    RESULTS
  end
